...
struct
  type roots_t = {
    p : Complex.t; q : Complex.t; r : Complex.t; s : Complex.t
  }
  ...
  let iterate (poly : Quartic.t) (rts : roots_t) =
    let open Complex in
    let p = sub rts.p (div (Quartic.compute poly rts.p) (List.fold_left mul (sub rts.p rts.q) [(sub rts.p rts.r); (sub rts.p rts.s)])) in
    let q = sub rts.q (div (Quartic.compute poly rts.q) (List.fold_left mul (sub rts.q p)     [(sub rts.q rts.r); (sub rts.q rts.s)])) in
    let r = sub rts.r (div (Quartic.compute poly rts.r) (List.fold_left mul (sub rts.r p)     [(sub rts.r q);     (sub rts.r rts.s)])) in
    let s = sub rts.s (div (Quartic.compute poly rts.s) (List.fold_left mul (sub rts.s p)     [(sub rts.s q);     (sub rts.s r)])) in
    {p; q; r; s}

  let rec find_roots_aux (poly : Quartic.t) (rts : roots_t) =
    let rts' = iterate poly rts in
    if roots_diff_epsilon rts rts' epsilon then
      rts'
    else (
      find_roots_aux poly rts')
